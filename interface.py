...
def createGUI():
    ...
    label_zip_file = tk.Label(root, text="Zip File:", fg='white')
    ...
    label_csv_file = tk.Label(root, text="Exported CSV File:",fg='white')
    label_csv_file.grid(row=4, column=2, sticky="e")
    label_csv_file.config(bg="#171810")
    entry_csvfile_path = tk.Label(root, borderwidth=2, relief="sunken")
    entry_csvfile_path.grid(row=4, column=3, sticky="nswe")
    button_browse_csv = tk.Button(root, text="Browse", highlightbackground="#171810", command = lambda : open_csv(entry_csvfile_path))
    button_browse_csv.grid(row=4, column=4)

    label_testing_file = tk.Label(root, text="JUnit Testing File:", fg='white')
    label_testing_file.grid(row=5, column=2, sticky="e")
    label_testing_file.config(bg="#171810")
    testing_file_path = tk.Label(root, borderwidth=2, relief="sunken")
    testing_file_path.grid(row=5, column=3, sticky="nswe")
    ...
    button_browse_testing.grid(row=5, column=4)

    addConstraintLabel = tk.Label(root, text="Add Constraints:", fg='white')
    addConstraintLabel.grid(row=6, column=2, sticky="e")
    addConstraintLabel.config(bg="#171810")
    entry_constraints_path = tk.Label(root, borderwidth=2, relief="sunken")
    entry_constraints_path.grid(row=6, column=3, sticky="nswe")
    ...
    button_browse_constraints.grid(row=6, column=4)
    
    addConstaintFormatLabel=tk.Label(root, text="Constraints Format: Keyword, Function Name, Limit, Points Deducted",fg='white')
    addConstaintFormatLabel.grid(row=7, column=3)
    ...
    button_process.grid(row=8, column=3)
...
def open_csv(label):
    csv_path = fd.askopenfile(mode ='rb', title='Choose Exported CSV file', filetypes = (("CSV files", "*.csv*"),))
    label.config(text = csv_path)
    return csv_path
